...
const Navbar = () => {
    ...
    return (
        <>
            <header className={`text-white py-3 sm:py-3 md:py-2 lg:py-2.5 xl:py-3 ${location.pathname === "/dashboard" || location.pathname === "/products" || location.pathname === "/shipping-info" || location.pathname === "/payment-info" || location.pathname === "/order-placed" ? "hidden" : "sticky mlg:fixed lg:w-full lg:bg-white lg:shadow"} z-20 top-0 shadow bg-white xl:max-w-[1400px] mx-auto`}>
                <div className=" mx-auto grid grid-cols-12 gap-y-2 justify-between items-center px-4 lg:max-h-12">
                    ...
                    <nav onClick={() => setIsOpenMainMenu(false)} className={`w-full sm:w-full md:w-auto md:col-span-9 ${isOpenMainMenu ? "flex sm:flex" : "hidden sm:hidden"} md:flex justify-end fixed sm:fixed md:relative h-full z-30 bottom-0 right-0  bg-gray-100 md:bg-white sm:bg-gray-100`} style={{ background: `${isOpenMainMenu ? "rgba(192, 191, 191, 0.347)" : "transparent"}` }}>
                        ...
                        <ul onClick={(e) => e.stopPropagation()} className="flex justify-start sm:justify-start md:justify-end flex-col md:items-center sm:flex-col md:flex-row gap-y-2  w-5/6 sm:w-5/6 md:w-auto bg-white sm:bg-white md:bg-white h-full sm:h-full md:h-auto py-8 sm:py-8 px-3 sm:px-3 md:px-0 md:py-0 relative">
                            ...
                            <hr className="mt-5 mt-6 md:hidden" />
                            ...
                            <li onClick={()=> navLinksActivate(0)} className=" mt-5 sm-mt-6 md:mt-0 p-0 flex items-center justify-center nav-link w-full md:w-max ">
                                <NavLink to="/" className="w-full text-start sm:text-start md:text-center sm:w-full md:w-auto text-[14px] md:text-[16px] border-0 md:border-b-2 md:border-blue-600 md:text-blue-600 text-white bg-blue-600 md:bg-white  hover:border-b-2 hover:border-b-2 hover:border-blue-600 font-semibold px-2 lg:px-3 py-0.5">Home</NavLink>
                            </li>
                            <li onClick={()=> navLinksActivate(1)} className="relative flex items-center justify-center nav-link w-full md:w-max px-0.5">
                                <NavLink to="/products" state={{productsType : "all", other : "all"}} className="w-full text-start sm:text-start md:text-center sm:w-full md:w-auto text-[14px] md:text-[16px] border-0 md:border-b-2 md:border-white md:text-blue-600 md:bg-white hover:border-blue-600 font-semibold text-theme-blue-600 px-2 lg:px-3 py-0.5" style={{transition:"all ease 0.1s"}}>Vegetables</NavLink>
                            ...
                            <li onClick={()=> navLinksActivate(2)} className="nav-link flex items-center justify-center w-full md:w-max px-0.5">
                                <button className="w-full text-start sm:text-start md:text-center sm:w-full md:w-auto text-[14px] md:text-[16px] border-0 md:border-b-2 md:border-white md:text-blue-600 md:bg-white hover:border-blue-600 font-semibold text-theme-blue-600 px-2 lg:px-3 py-0.5">About</button>
                            </li>
                            <li onClick={()=> navLinksActivate(3)} className="nav-link flex items-center justify-center w-full md:w-max">
                                <button className="w-full text-start sm:text-start md:text-center sm:w-full md:w-auto text-[14px] md:text-[16px] border-0 md:border-b-2 md:border-white md:text-blue-600 md:bg-white hover:border-blue-600 font-semibold text-theme-blue-600 px-2 lg:px-3 py-0.5">Contact</button>
                            </li>
                            ...
                            } className=" relative md:w-[60px] lg:w-20 xl:w-20 hidden md:block mx-2">
                          <span className="block md:hidden"> 
                            ...
                            <span className="flex items-center rounded w-full text-start sm:text-start md:text-center w-max text-[14px] font-semibold text-white bg-theme-blue-600 pl-2 lg:px-3 py-0.5 pb-1">
                                Cart
                    ...
                    <div className="md:order-2 col-span-12 sm:col-span-12 md:col-span-12 flex flex-row justify-end sm:justify-end md:justify-start fixed sm:fixed lg:relative bottom-6 h-max bottom-6 md:top-[14vh] lg:top-[10px] lg: right-4 md:left-0">
                        <Search 
                        fieldCss="shadow-md h-10 sm:h-10 md:h-10 lg:h-12 xl:h-12 w-[80%] md:w-[30%] max-w-[300px] md:left-0 z-10 relative rounded-full"
                        ...
                        <NavLink to={`${auth ? "/profile" : "/login"}`} onClick={() => {
                            setIsOpenMainMenu(false)}
                            } className="ml-auto aspect-square w-[40px] md:w-[40px] lg:w-[50px] aspect-square md:bg-white md:shadow flex justify-center items-center p-2 rounded-full mr-2 bg-white">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-[35] xl:w-[40px] text-theme-blue-600 md:text-gray-300 ">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                            </svg>
                        </NavLink>
                    </div>

                    <div className="order-2 ms:order-2 md:order-3 col-span-9 sm:col-span-9 md:col-span-4 flex justify-end gap-x-1 sm:gap-x-2.5 md:gap-x-3.5 lg:gap-x-4 xl:gap-x-5">
