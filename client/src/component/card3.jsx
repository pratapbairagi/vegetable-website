...
const Card3 = ({product}) => {
    ...
    return (
        <div className="card col-span-6 sm:col-span-5 md:col-span-4 lg:col-span-3 h-full shadow-sm hover:shadow-md lg:h-max min-h-30vh min-w-[50%] md:min-w-[340px] w-max md:max-w-[50%] lg:max-w-[35%] flex flex-col lg:flex-row py-2 pb-1">
            <div className="h-3/6 lg:h-60 w-full lg:w-[45%] relative">
            
                <SellerProductTag containerClass="top-0 md:top-2 right-1 md:right-2" seller={product.seller} productId={product._id}/>
               
                <img src={product.images[0]?.url} className="w-full h-full object-contain mx-auto" alt="" />
                <NavLink to={`/vegetable/${product._id}`} className="absolute top-1 md:top-2 left-2 md:left-4 text-xs sm:text-sm font-nunito text-theme-blue-600 md:text-base lg:text-lg font-bold capitalize max-w-[100%] word-break">{product.title}</NavLink>
            ...
            <div className=" lg:w-[55%] lg:px-4 flex flex-col justify-between lg:py-2 lg:min-w-">
            <h5 className="w-full flex items-center gap-x-4 px-2">
                <span className="text-base sm:text-base font-extrabold text-theme-green-600 font-nunito ">₹ {product.price}/KG</span>
               {product.features && product.features.feature == "discount" &&  <span className="text-xs sm:text-xs font-bold line-through text-red-400 ">₹ {product.price + (product.price*product.features.value/100)}/KG</span> } 
            </h5>
            ...
            <p className="px-2 text-2xs sm:text-2xs md:text-xs lg:text-sm xl:text-base min-h-12 line-clamp-3 lg:line-clamp-5 lg:min-h-24 text-gray-400 mt-2">{product.description}</p>
            {
                            ...
                            : 
                            <button onClick={()=> dispatch(add_to_cart(product._id))} className="text-3xs sm:text-3xs md:text-2xs lg:text-xs font-semibold text-theme-blue-600  px-1.5 sm:px-1.5 md:px-2.5 lg:px-3.5 xl:px-4 py-2 sm:py-2 md:py-2.5 lg:py-3 rounded-sm hover:bg-white hover:border-transparent  mt-4 mx-auto border" style={{ width: "96%", justifySelf:"end" }}>ADD TO CART</button>
