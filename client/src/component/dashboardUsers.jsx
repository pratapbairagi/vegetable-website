...
import {get_users} from "../redux/user/action"




// const DashboardProducts = ({products, searchQueries, setSearchQueries, dispatch}) =>{
    const DashboardUsers = ({type="Sellers"}) =>{
    const {user, users, loading, success, error, message, totalUsersLength} = useSelector(state => state.user)
    ...
    const fetchProducts = ()=> {
        // if( !products.length && x == 1 && location.state == null){
            ...
            dispatch(get_users({pageNo : searchQueries.pageNo, limit : searchQueries.limit, search : searchQueries.search, type}))
            // }
    ...
    const submitSearchClear_fun = () => {
        dispatch(get_users({pageNo : searchQueries.pageNo, limit : searchQueries.limit, search : ""}))
    }
    ...
    return (
        <div className="w-full col-span-12 max-w-full h-max min-h-screen max-w-full flex flex-col md:mb-0 ">
                ...
                <table className="w-max min-w-full overflow-x-auto min-w-full  max-h-50vh divide-y divide-gray-200 ">
                    <thead className="sticky top-0  bg-white w-max">
                        <tr className="flex gap-x-4 sm:gap-x-6 md:gap-x-8 lg:gap-x-10 xl:gap-x-14">
                            <th className="px-2 w-28 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">USER ID</th>
                            <th className="px-2 w-28 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">USER NAME</th>
            ...
            <Pagination fun={(e)=> setSearchQueries(prev=> ({...prev, pageNo : e})) } activePage={searchQueries.pageNo} numbersOfButton={(totalUsersLength / searchQueries.limit)} />
