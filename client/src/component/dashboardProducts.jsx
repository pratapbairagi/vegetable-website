...
import { delete_product, get_filter_and_sort_products, get_seller_filter_products } from "../redux/product/action"
...
const DashboardProducts = () => {
    const { sellerProducts, sellerProductsLength } = useSelector(state => state.product)
    ...
    const fetchProducts = () => {
        ...
        // if(  x == 0 ){
        x++
        dispatch(get_seller_filter_products({
    ...
    let productSearchInput_fun = useRef()

    const submitSearchClear_fun = () => {
        dispatch(get_seller_filter_products({ pageNo: 1, limit: searchQueries.limit, search: "" }))
    }

    useEffect(() => {
        fetchProducts()
    }, [searchQueries.pageNo, searchQueries.search])
        ...
        <div className="w-full col-span-12 max-w-full h-max min-h-screen max-w-full flex flex-col md:mb-0 ">
            ...
            <div className=" w-full text-base md:text-xl lg:text-lg xl:text-xl font-bold text-gray-600 flex justify-between bg-white py-2 px-2 md:px-4 md:py-3 mt-2 sm:mt-3 md:mt-3 lg:mt-4 xl:mt-4">
                <fieldset className=" h-8 sm:h-8 md:h-10 lg:h-10 xl:h-12 w-2/4 sm:w-2/4 md:w-3/4 lg:w-1/4  sm-left-125 md:left-0 z-0 rounded left-0 relative" >
                    <input ref={productSearchInput_fun} className="block w-full h-full px-6 rounded text-sm bg-gray-100" type="text" placeholder="Search for..." name="" id="" />
                    <button onClick={() => setSearchQueries(prev => ({ ...prev, search: productSearchInput_fun.current.value }))} className="absolute top-0 right-0 h-full text-white grid place-items-center w-10 sm:w-10 md:w-12 lg:w-16 xl:w-16">
            ...
            <Pagination fun={(e) => setSearchQueries(prev=> ({...prev, pageNo : e}))} activePage={searchQueries.pageNo} numbersOfButton={(sellerProductsLength / searchQueries.limit)} />
